...
// Adiciona uma fonte (densidade ou velocidade) ao campo correspondente
void add_source(int N, float *x, float *s, float dt) {
    for (int i = 0; i < (N + 2) * (N + 2); i++) x[i] += dt * s[i];
}

// Define as condições de contorno para o campo (densidade ou velocidade)
...
// Calcula a posição anterior da partícula
// limites
...
void idle() {
    ...
    for (int i = 0; i <= N; i++) {
        for (int j = 0; j <= N; j++) {
            dens[IX(i, j)] *= 0.91f;  // Reduz a densidade a cada iteração (ajuste o fator para controlar a velocidade)
        }
    }
