...
const float diff = 0.00000000000000001f; // Difusão ajustada para maior fluidez
const float visc = 0.00000000000000001f; // Viscosidade menor para fluidez


const float dt = 0.092f;
...
void idle() {
    if (mouse_down) {
        ...
        dens[IX(i, j)] += 1500.0f;
        u[IX(i, j)] += (rand() % 10 - 5) * 0.5f; // Perturbação horizontal aleatória
        v[IX(i, j)] += -20.0f;                  // Força vertical para baixo
    }

    // Gravidade proporcional à densidade acumulada
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= N; j++) {
            v[IX(i, j)] -= 0.1f + dens[IX(i, j)] * 0.0001f; // Gravidade ajustada
        }
    }
