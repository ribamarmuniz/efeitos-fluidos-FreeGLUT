...
void cyclone(int cx, int cy, float strength) {
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= N; j++) {
            ...
            if (dist == 0) continue; // Evitar divisão por zero.

            // Reduz a força com a distância e limita o efeito para áreas próximas.
            float force = strength / (dist + 5); // Ajuste o divisor para controlar o alcance.

            // Gira o vetor em torno do cursor (ângulo ajustado para suavidade).
            float angle = atan2(dy, dx) + M_PI / 2; // Gira 90 graus.

            // Adiciona velocidades circulares, suavizando o efeito.
            u[IX(i, j)] += force * cos(angle) * 0.03f; // Fator 0.2f reduz a velocidade.
            v[IX(i, j)] += force * sin(angle) * 0.02f;
        }
    }
}
